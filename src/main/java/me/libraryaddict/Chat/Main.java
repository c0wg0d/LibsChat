...
public class Main extends JavaPlugin {
    ...
    public void addToChatSpies(Player player) {
        if (!chatspies.contains(player))
            chatspies.add(player);
    }

    public void removeFromChatSpies(Player player) {
        if (chatspies.contains(player))
            chatspies.remove(player);
    }

    public boolean isChatSpy(Player player) {
        return chatspies.contains(player);
    }
    ...
    public ChatChannel getAliveChannel() {
        return aliveChannel;
    }
    ...
    public void loadConfig() {
        ...
        aliveChannel = null;
        if (config.contains("AliveChannel")) {
            for (ChatChannel channel : newChannels) {
                if (channel.getName().equals(config.getString("AliveChannel")))
                    aliveChannel = channel;
            }
        }
        ...
        if (baseConfig.contains("Commands")) {
            for (String key : baseConfig.getConfigurationSection("Commands").getKeys(false)) {
                ...
                try {
                    ...
                    } else if (key.equals("Chatspy")) {
                        registerCommand(commandName, new Chatspy(main), this, aliases, false);
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
